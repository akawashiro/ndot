...
pub fn generate_svg(graph: &Graph) -> String {
    ...
    let node_positions = if is_dag_result {
        ...
        calculate_sugiyama_positions(graph)
    } else {
        // Use circular layout for non-DAGs
        info!("generate_svg: Using circular layout for non-DAG");
        calculate_circular_positions(graph)
    };
    
    info!("generate_svg: Node positions calculated in {:?}", start_time.elapsed());
...
fn is_dag(graph: &Graph) -> bool {
    ...
    if graph.edges.iter().any(|e| !e.is_directed) {
        info!("is_dag: Found undirected edge, not a DAG. Took {:?}", start_time.elapsed());
        return false;
    }
    ...
    info!("is_dag: Adjacency list created with {} entries", adj_list.len());
    ...
    fn has_cycle(
        ...
        if let Some(neighbors) = adj_list.get(node) {
            debug!("has_cycle: Node {} has {} neighbors", node.id.name, neighbors.len());
            for &neighbor in neighbors {
                // If neighbor not visited, check if it leads to a cycle
                if !visited.contains(&neighbor.id.name) {
                    debug!("has_cycle: Checking unvisited neighbor {}", neighbor.id.name);
                ...
                // If neighbor is in current recursion path, we found a cycle
                    ...
                    debug!("has_cycle: Found cycle - {} is already in path", neighbor.id.name);
                    return true;
                }
            }
        }
    ...
    for node in &graph.nodes {
        if !visited.contains(&node.id.name) {
            ...
            if has_cycle(node, &adj_list, &mut visited, &mut path) {
                info!("is_dag: Found cycle, not a DAG. Took {:?}", start_time.elapsed());
                return false;
            }
        }
    }
    
    info!("is_dag: No cycles found, is a DAG. Took {:?}", start_time.elapsed());
    true
}

// Function to calculate node positions using the Sugiyama algorithm for DAGs
fn calculate_sugiyama_positions(graph: &Graph) -> HashMap<Node, (i32, i32)> {
    ...
    info!("calculate_sugiyama_positions: Layers assigned in {:?}, {} layers created", 
          layer_start.elapsed(), layers.len());
    ...
    info!("calculate_sugiyama_positions: Crossings minimized in {:?}", crossing_start.elapsed());
...
fn assign_layers(graph: &Graph) -> Vec<Vec<Node>> {
    ...
    info!("assign_layers: In-degree calculated for {} nodes", in_degree.len());
    ...
    for node in &graph.nodes {
        if in_degree.get(&node.id.name).unwrap_or(&0) == &0 {
            info!("assign_layers: Node {} has no incoming edges, adding to first layer", node.id.name);
    ...
    while !queue.is_empty() {
        ...
        info!("assign_layers: Processing node {} in layer {}", node.id.name, current_layer);
        ...
        // Find outgoing edges
        let outgoing = graph.edges.iter()
            .filter(|e| e.source.id.name == node.id.name)
            .map(|e| e.target.clone())
            .collect::<Vec<_>>();
        
        info!("assign_layers: Node {} has {} outgoing edges", node.id.name, outgoing.len());
        ...
        for target in outgoing {
            ...
            *in_deg -= 1;
            ...
            info!("assign_layers: Reduced in-degree of {} to {}", target.id.name, in_deg);
            
            if *in_deg == 0 {
                info!("assign_layers: Adding {} to queue (in-degree is now 0)", target.id.name);
        ...
        if queue.is_empty() && node_to_layer.len() < graph.nodes.len() {
            ...
            for node in &graph.nodes {
                if !node_to_layer.contains_key(&node.id.name) {
                    info!("assign_layers: Adding unprocessed node {} to queue", node.id.name);
                    queue.push_back(node.clone());
                    break;
                }
            }
    ...
    info!("assign_layers: Processed {} nodes in {} layers", processed_count, layers.len());
    
    // Remove empty layers
    layers.retain(|layer| !layer.is_empty());
    info!("assign_layers: After removing empty layers, {} layers remain", layers.len());
    ...
    info!("assign_layers: Layer optimization completed in {:?}", optimize_start.elapsed());
    
    info!("assign_layers: Layer assignment completed in {:?}", start_time.elapsed());
    layers
}
...
fn optimize_layers(graph: &Graph, layers: &mut Vec<Vec<Node>>) {
    ...
    info!("optimize_layers: Created node-to-layer map with {} entries", node_to_layer.len());
    ...
    while changed {
        ...
        for layer_idx in 0..layers.len() {
            info!("optimize_layers: Processing layer {}/{}", layer_idx + 1, layers.len());
            let mut i = 0;
            while i < layers[layer_idx].len() {
                let node = &layers[layer_idx][i];
                debug!("optimize_layers: Processing node {} in layer {}", node.id.name, layer_idx);
                ...
                debug!("optimize_layers: Node {} has {} incoming and {} outgoing connections", 
                      node.id.name, incoming.len(), outgoing.len());
                ...
                if !incoming.is_empty() && !outgoing.is_empty() {
                    ...
                    if median != layer_idx && 
                       (median > layer_idx && incoming.iter().all(|&l| l < median) ||
                        ...
                        best_layer = median;
                        debug!("optimize_layers: Median layer {} is better for node {}", median, node.id.name);
                    }
                    ...
                    if max_incoming + 1 != layer_idx {
                        ...
                        debug!("optimize_layers: Layer {} is better for node {} (incoming edges only)", 
                              best_layer, node.id.name);
                    }
                } else if !outgoing.is_empty() {
                    // If node only has outgoing edges, place it as high as possible
                    let min_outgoing = *outgoing.iter().min().unwrap_or(&layers.len());
                    if min_outgoing > 0 && min_outgoing - 1 != layer_idx {
                        best_layer = min_outgoing - 1;
                ...
                if best_layer != layer_idx {
                    info!("optimize_layers: Moving node {} from layer {} to {}", 
        ...
        info!("optimize_layers: Iteration {} completed in {:?}, changed: {}", 
             iteration, iter_start.elapsed(), changed);
        
        // Remove empty layers
        let before_len = layers.len();
        layers.retain(|layer| !layer.is_empty());
        info!("optimize_layers: Removed {} empty layers", before_len - layers.len());
    ...
    info!("optimize_layers: Layer optimization completed in {:?} after {} iterations", 
         start_time.elapsed(), iteration);
}
...
fn minimize_crossings(graph: &Graph, layers: &Vec<Vec<Node>>) -> HashMap<Node, i32> {
    ...
    info!("minimize_crossings: Initialized positions for {} nodes", x_positions.len());
    ...
    for edge in &graph.edges {
        outgoing.entry(edge.source.id.name.clone())
                .or_insert_with(Vec::new)
                .push(edge.target.clone());
                
        incoming.entry(edge.target.id.name.clone())
                .or_insert_with(Vec::new)
                .push(edge.source.clone());
    }
    
    info!("minimize_crossings: Created adjacency lists with {} outgoing and {} incoming entries", 
...
fn center_layout(positions: &mut HashMap<Node, (i32, i32)>) {
    info!("center_layout: Starting layout centering for {} nodes", positions.len());
    let start_time = Instant::now();
    ...
    info!("center_layout: Bounds calculated - width: {}, height: {}, offsets: ({}, {})",
    ...
    info!("center_layout: Layout centered in {:?}", start_time.elapsed());
}

// Function to calculate node positions using circular layout (for non-DAGs)
fn calculate_circular_positions(graph: &Graph) -> HashMap<Node, (i32, i32)> {
    info!("calculate_circular_positions: Starting circular layout for {} nodes", graph.nodes.len());
    let start_time = Instant::now();
    ...
    if node_count > 0 {
        ...
        info!("calculate_circular_positions: Using radius {}, center ({}, {})", 
    ...
    info!("calculate_circular_positions: Circular layout completed in {:?}", start_time.elapsed());
    positions
}

// Function to export the graph as an SVG string
pub fn graph_to_svg(graph: &Graph) -> String {
    info!("graph_to_svg: Starting SVG generation for graph with {} nodes and {} edges", 
          graph.nodes.len(), graph.edges.len());
    let start_time = Instant::now();
    
    let result = generate_svg(graph);
    
    info!("graph_to_svg: SVG generation completed in {:?}, SVG size: {} bytes", 
